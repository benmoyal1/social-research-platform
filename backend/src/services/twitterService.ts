 ...
 */
export async function getPaginatedTwitterPosts(
  page: number = 1,
  limit: number = 50,
  filters: TwitterFilters = {}
): Promise<PaginationResult<TwitterPost>> {
  ...
  try {
    ...
    `;
    const dataResult: QueryResult<TwitterPost> = await client.query(dataQuery, [...values, limit, offset]);
 ...
 */
export async function streamAllTwitterPosts(): Promise<QueryStream> {
  const client = await pool.connect();
 ...
 */
export async function streamFilteredTwitterPosts(filters: TwitterFilters): Promise<QueryStream> {
  const client = await pool.connect();
  const conditions: string[] = [];
  const values: any[] = [];
  let paramCount = 1;
 ...
 * Get available users
 */
export async function getTwitterUsers(): Promise<string[]> {
  const result: QueryResult = await pool.query('SELECT DISTINCT user_posted FROM twitter_posts ORDER BY user_posted');
  return result.rows.map((row: any) => row.user_posted);
}

/**
 * Get statistics
 */
export async function getTwitterStats(): Promise<any> {
