...
import { TwitterFilterInputsProps } from "../../types";
import "./css/TwitterFilterinputs.css";
import { twitterUniqueUsers } from "../../utils/twitterUniqueUsers";

const TwitterFilterInputs: React.FC<TwitterFilterInputsProps> = ({
  filters,
  handleChange,
  ...
  const tooltipRef = useRef<HTMLSpanElement>(null); // Reference for the tooltip
  ...
  const handleKeyDown = (
    feature: string,
    e: React.KeyboardEvent<HTMLInputElement> | React.FormEvent<HTMLInputElement>,
    setStateFun: (value: string) => void,
    featureInput: string
  ): void => {
    console.log(e.type === "input");
    if ((e.type === "keydown" && (e as React.KeyboardEvent).key === "Enter" || e.type === "input") && featureInput.trim()) {
  ...
  return (
    <div className="twitter-filter-container">
      ...
      <div className="username-dropdown">
        {filters.user_posted && filters.user_posted.map((user, index) => (
          <div
            key={index}
      ...
      {filters.tagged_users && filters.tagged_users.length > 0 && (
        <div className="hashtags-dropdown">
